...
init_by_path:: proc(pipe: ^External_Pipe, path: string, mode: int) -> (err: os.Error) {
	...
	pipe.handle = auto_cast windows.CreateNamedPipeW(auto_cast &path_u16[0], (mode == os.O_RDONLY) ? windows.PIPE_ACCESS_INBOUND : windows.PIPE_ACCESS_OUTBOUND, windows.PIPE_TYPE_MESSAGE | windows.PIPE_READMODE_MESSAGE | windows.PIPE_WAIT, 1, 65_536, 65_536, 0, nil)
