...
determine_dependers:: proc(cell_B: ^Cell) {
	...
	for cell_A_id, _ in session.cells do if cell_A_id != cell_B.frontend_cell_id do for deprule in deprules {
...
recompile_dependers:: proc(cell_B: ^Cell) {
	for &cell_A, i in cell_B.weak_dependers do if slice_contains_cell(cell_B.strong_dependers[:], cell_A) {
		recompile_cell(cell_B.session, cell_A.frontend_cell_id, cell_A.code_raw) }
	for &cell_A in cell_B.strong_dependers {
		recompile_cell(cell_B.session, cell_A.frontend_cell_id, cell_A.code_raw)
		free_all(cell_A.dll_context.allocator)
		run_cell(cell_A) }
